(function () {
    ...
    /**
     * СТИЛІ ІНТЕРФЕЙСУ (CSS) - З КАХКАДНОЮ АНІМАЦІЄЮ БЕЙДЖІВ
     */
    function applyStyles() {
        ...
        // Анімація для послідовної появи кожного бейджа якості (Cascade Stagger)
        ...
        // Послідовна каскадна анімація бейджиків якості
        css += '.quality-item { height: 1.1em; filter: drop-shadow(0 2px 4px rgba(0,0,0,0.3)); ';
        if (isUIAnim) {
            css += 'opacity: 0; animation: quality_badge_cascade 0.5s cubic-bezier(0.16, 1, 0.3, 1) forwards; will-change: transform, opacity, filter; ';
        }
        css += '} ';
        
        // Динамічна затримка (stagger) для кожного бейджа
        css += '.quality-item:nth-child(1) { animation-delay: 0.40s; } ';
        css += '.quality-item:nth-child(2) { animation-delay: 0.50s; } ';
        css += '.quality-item:nth-child(3) { animation-delay: 0.60s; } ';
        css += '.quality-item:nth-child(4) { animation-delay: 0.70s; } ';
